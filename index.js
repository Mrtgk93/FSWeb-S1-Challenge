...
function indekseGoreFenomen(feno,x) {
 let a=x + ". indekste bulunan fenomen: " + feno[x].profile;
  return a;
}
...
function fenomenEkle(feno,sayi,isim,takipci,gönderi,platfor) {
 let a=[...feno];
 let b= { number: sayi,
 profile: isim,
 followers: takipci,
...
console.log(fenomenEkle(fenomenler,6,'Mert',34157906,12,'Instagram'));
...
function platformaGoreCokGonderiYapanFenomen(feno,platfor){
  let a=[];
  let b=[];
  for (let i = 0; i < feno.length; i++) {
